-- Backup: public.save_own_profile() definition before D6 §4 state-gate change
-- Captured 2026-07-29 via pg_get_functiondef on production (oqrxdvwlsbwkhihsvqvt)
-- Rollback: re-run this CREATE OR REPLACE to restore the hard-block behavior.
...
-- protect_profile_columns() also needed changing (see migration
-- 20260729000000) since it independently blocks any non-admin UPDATE that
-- changes verification_status/is_public/approved_at outside the one
-- draft|rejected -> pending case it already whitelisted for submit_profile().
-- Backing up its prior definition here too.
