...
- 과도한 장식 금지
- 기능보다 정보 위계 우선
- 자격 간 서열화 금지 — 카테고리는 구분하되 우열을 암시하는 시각적 위계(색상/크기 차등)를 두지 않는다
...
자격 카테고리 (MVP: 텍스트 라벨만, 카테고리별 디자인 차별화는 P1로 연기 — 11_BACKLOG.md):
- 국가면허
- 국가자격
- 민간자격
- 교육수료
...
카테고리 배지는 우열이 아닌 구분 표시다 — MVP에서도, P1 디자인 차별화 이후에도 색상/크기로 우열을 암시하지 않는다.
