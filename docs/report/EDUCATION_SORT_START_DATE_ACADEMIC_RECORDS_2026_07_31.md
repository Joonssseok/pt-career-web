...
- 대학교/대학원 구분 선택 시 실제 대학명(가천대학교, 가톨릭대학교 등) 검색 결과가 로컬 데이터로 즉시 반환되는지 JSON/필터 로직 자체는 스크립트로 확인함(브라우저 실제 재현은 별도 테스트 계정 이메일 레이트리밋으로 이번엔 생략 — 다음 프로덕션 적용 시 실제 계정으로 재확인 예정).
