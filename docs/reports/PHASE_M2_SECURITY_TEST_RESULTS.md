...
**실행**:
```sql
-- TEST_EXPERT_A의 draft profile 조회 시도
SELECT * FROM profiles WHERE user_id = '<TEST_EXPERT_A_UUID>';
```
...
**사전 데이터**: TEST_EXPERT_A (draft), TEST_EXPERT_B (approved+public)

**실행**:
```sql
-- anon이 approved+public profile만 조회 가능한지 확인
...
UPDATE profiles SET verification_status = 'approved' WHERE user_id = '<TEST_EXPERT_A_UUID>';
UPDATE profiles SET is_public = true WHERE user_id = '<TEST_EXPERT_A_UUID>';
UPDATE profiles SET approved_at = NOW() WHERE user_id = '<TEST_EXPERT_A_UUID>';
...
-- TEST_EXPERT_A의 license verification_status 변경 시도
UPDATE licenses SET verification_status = 'verified' 
WHERE profile_id IN (SELECT id FROM profiles WHERE user_id = '<TEST_EXPERT_A_UUID>');
...
-- TEST_EXPERT_B가 TEST_EXPERT_A의 profile 수정 시도
...
-- TEST_EXPERT_A가 자신을 admin으로 등록 시도
...
-- TEST_EXPERT_A가 admin_actions 조회 시도
