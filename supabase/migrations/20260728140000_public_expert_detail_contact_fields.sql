-- Expose direct-contact workplace fields on public_expert_detail (2026-07-28)
-- MVP core value #4: no booking/payment, just direct contact via address/phone/
-- website/external contact. Gated by the same is_location_public flag as the
-- existing region/center_name/website_url fields. public_expert_list is out
-- of scope (list page doesn't need these fields).
...
CREATE OR REPLACE VIEW public.public_expert_detail AS
 SELECT p.id,
    ...
    COALESCE(lic.licenses, '[]'::jsonb) AS licenses,
        CASE WHEN w.is_location_public THEN w.address ELSE NULL END AS workplace_address,
        CASE WHEN w.is_location_public THEN w.address_detail ELSE NULL END AS workplace_address_detail,
        CASE WHEN w.is_location_public THEN w.phone ELSE NULL END AS workplace_phone,
        CASE WHEN w.is_location_public THEN w.external_contact_url ELSE NULL END AS workplace_external_contact_url,
        CASE WHEN w.is_location_public THEN w.latitude ELSE NULL END AS workplace_latitude,
        CASE WHEN w.is_location_public THEN w.longitude ELSE NULL END AS workplace_longitude
   FROM profiles p
